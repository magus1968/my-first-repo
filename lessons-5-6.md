...
`45:30` В браузере переходим в новую ветку -> жмем `Contribute` -> выбираем `Open pull request` -> GitHub анализирует то что мы сделали и выдает сообщение `Able to merge` - о возможности слить то что сделали мы с тем что есть в основном (*форкнутом*) репозитории - т.е. предложить владельцу *форкнутого* репозитория наши изменения
