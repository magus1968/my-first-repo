...
> ПРИМЕЧАНИЕ: у меня вместо этой кнопки появились две кнопки `Contribute` и `Sync fork`. Причем команда (кнопка) `Open pull request` активна при нажатии `Contribute` только из новой ветки, в которой мы сформировали файл со своим предложением.
...
`13:25` При необходимости, вносим изменения в README.md *(с последующим сохранением, индексацией и коммитом)*
...
git pull
```
`19:55` C помощью `git log` посмотрим количество коммитов; кто автор коммитов; что голова `HEAD` указывает на ветку `main` и удаленный репозиторий `origin` тоже указывает на ветку `main`. Хорошо
...
`26:05` Переключаемся в консоли на ветку `main` - сделаем слияние `git merge newbranch`. Слияние произошло - смотрим `git log --oneline --graph`: *main* и *newbranch* на одном уровне. Отлично
...
`28:05` Удалить ветку можно *через браузер:* выбрать в раскрывающемся списке main `View all branches` и нажать на корзину для `new_branch`.
...
```sh
[rejected] # Отказано
error: failed to push some refs to 'https://github.com/magus1968/test_repo.git'
# Вы изменили устаревший репозиторий
```
Мы не можем вытолкнуть текущие изменения.

>! Перед тем как внести какие-то локальные изменения <— нужно сначала забрать то что есть на сервере (git pull)

`31:50` ОК, разруливаем ситуацию. Перед тем как отправить изменения на сервер, мы должны выполнить команду
```sh
git pull --rebase
```
...
`32:20` Получаем сообщение, что при автослиянии произошел КОНФЛИКТ. Мы не можем наши текущие изменения слить вместе с удаленным текстом.
```sh
Auto-merging README.md
CONFLICT (content): Merge conflict in README.md
error: could not apply 0246f80... Локальный конфликт: внесли изменения в readme из консоли
```
...
```
*PS: попробовал предложенный git'ом коммит с ключем `git commit -a`, но пока не понял*

`33:45` Теперь наш файл нужно добавить и закоммитить...

>*Но Денис Enter то ли нажал, то ли нет, а я нажал и расхлебывал последствия уже самостоятельно*. Поэтому совет мне: попробуй `БЕЗ правки вручную` и/или воспользуйся кнопкой `Resolve Editor`

`34:05` *Пошли по второму кругу.* Заходим в GitHub (браузер)-> cнова редактирум файл на GitHub -> после этого вносим изменения в локальный файл -> git status -> cохраняем локальный, добавляем к индексации, коммитим -> пытаемся сделать `git push` -> видим снова отказ `[rejected]`.

`35:30` Выполняем уже известный `git pull --rebase`, который позволит при выкачке данных с сервера наложить наши локальные изменения на серверные изменения -> снова видим `Merge conflict in README.md` -> вызвав `git status` снова увидим `both modified: README.md`

`36:15` Редактируем вручную файл в VSCode -> сохраняем, git status, git add, `git status` - до коммита -> и видим подсказку, что для продолжения rebase нужно ввести:
```sh
git rebase --continue
```
> *! После этого из консоли перекидывает в VSCode в какое-то новое окно -> пока не понял что там нужно/можно сделать и просто закрыл.*

Что-то этот полезный ключик `--continue` конечно сделал:
```sh
Successfully rebased and updated refs/heads/main.
```
Но только текст сохранился не полностью -> лан, разберёмся -> `git push` :)

> *Совет от Дениса: почаще набирать `git pull`*

Кстати, получил `[rejected]` внеся изменения в README.md на Github, а потом отредактировав lessons-5-6.md локально. `git --rebase` помог сразу (без --continue) - видимо потому что сливал два разных файла, а не изменения в одном. ИМХО

---

## 6.5 Тренируемся в pull request

`38:20` - Возьмем чей-то репозиторий на GitHub -> склонируем `Fork` его в *свой* аккаунт на GitHub -> предложим какие-то свои изменения владельцу *форкнутого* репозитория.

`39:40` Под своим аккаунтом заходим на *чужой* репозиторий -> нажимаем кнопку `Fork` -> склонированный репозиторий появляется в нашем аккаунте -> клонируем его себе через консоль на локальный компьютер: копируем ссылку -> `git clone` -> `ls` -> `cd` - заходим в созданную при клонировании папку -> `git status`

`42:00` В качестве эксперимента добавим, например, файл с инструкцией: скопируем и добавим в папку с репозиторием -> `git status` - видим, что файл появился.

`43:10` Добавляем новую ветку `git branch new_branch` -> `git branch` - проверяем, что она появилась -> `git checkout new_branch` - перейдем в новую ветку -> и вот теперь внесем изменения *в новой ветке* -> `git add` - `git commit`

`45:00` Выталкиваем новую ветку на сервер git push -> получаем подсказку `git push --set-upstream origin newbranch` -> копируем - вставляем - пушим

`45:30` В браузере переходим в новую ветку -> жмем `Compare & pull request` -> GitHub анализирует то что мы сделали и выдает сообщение `Able to merge` - о возможности слить то что сделали мы с тем что есть в основном (*форкнутом*) репозитории - т.е. предложить владельцу *форкнутого* репозитория наши изменения

`46:10` Добавляем описание нашего предложения для владельца форкнутого репозитория.

`46:35` Нажимаем `Create pull request` чтобы отправить pull request в аккаунт владельца исходного репозитория -> GitHub проверяет что никаких конфликтов нет и выдает сообщение `This branch has no conflicts with the base branch` -> pull request отправлен.

`46:45` Можно посмотреть как это будет выглядеть у владельца репозитория - Денис показывает на своем аккаунте: может принять предложение `Merge pull request` и слить с текущим состоянием или написать какой-то комментарий и обсудить предложение.

`49:05` Финальное задание: сделать `Fork` от репозитория `gulden-geekbrains/version_control` в свой GitHub -> склонировать себе на комп -> добавить туда инструкцию -> отправить себе в GitHub -> pull request - делаем предложение владельцу репозитория.

---

## 7. Полезно, пригодится
