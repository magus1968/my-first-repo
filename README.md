...
`45:55` С этой копией уже можем делать всё, что нам заблагорассудится. У нас есть к нему полный доступ, как-будто мы сами его создали
...
`53:00` Нажимаем `Open pull request`. GitHup анализирует то что мы сделали и выдает сообщение `Able to merge` - о возможности слить то что сделали мы и то что было в основном (форкнутом) репозитории.
