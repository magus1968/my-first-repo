...
`13:25` По необходимости можем внести изменения в README.md с последующим сохранением, индексацией и коммитом
...
`16:40` Убедимся, что у нас привязан удаленный репозиторий `origin` и как у нас привязан удаленный репозиторий с помощью команды 
```sh
git remote -v
```
Увидим
...
`19:00` Чтобы произведенные на GitHub изменения появились в локальном репозитории - запрашиваем эти изменения с сервера
```sh
git pull
```
...
`22:45` Промоделируем ветку: попробуем создать новую ветку, чтобы вытолкнуть на GitHub только её.
